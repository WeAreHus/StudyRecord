...
def on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:         #左键点击
        ...
        cv2.rectangle(img2, point1, (x,y), (255,0,0), 8) # 图像，矩形顶点，相对顶点，颜色，粗细
        cv2.imshow('image', img2)
    elif event == cv2.EVENT_LBUTTONUP:         #左键释放
        point2 = (x,y)
        cv2.rectangle(img2, point1, point2, (0,0,255), 10) 
        cv2.imshow('image', img2)
        ...
        cv2.imwrite('file_path(图片处理后保存地址)', thresh_img)  # 预处理后图像保存位置

def main():
    global img
    img = cv2.imread('file_path(图片地址)')  # 手写数字图像所在位置
